...
class LongTermMemory:
    ...
    def _migrate_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "preferences" not in data:
            data["preferences"] = []
        if "memory_summary_cache" not in data or not isinstance(data.get("memory_summary_cache"), dict):
            data["memory_summary_cache"] = {
                "summary": "",
                "updated_at": "",
    ...
    def get_summary_cache(self) -> Dict[str, Any]:
        """Return the last successful long-term memory LLM summary cache."""
        cache = self.data.get("memory_summary_cache")
        if not isinstance(cache, dict):
            cache = {"summary": "", "updated_at": "", "source_message_count": 0}
            self.data["memory_summary_cache"] = cache
        return dict(cache)

    def save_summary_cache(self, summary: str, source_message_count: int = 0):
        """Persist the last successful long-term memory LLM summary."""
        text = str(summary or "").strip()
        if not text:
            return
        self.data["memory_summary_cache"] = {
            "summary": text,
            "updated_at": datetime.now().isoformat(),
