...
【结构化路线优先】
如果前序智能体已经提供 `route_planning` 或 `route_options`：
1. 必须优先使用其中的 POI 顺序、到达/离开时间、交通时间、排队等级和预算指标
2. 不要擅自替换为未出现在 `route_options` 中的主要 POI
3. 可以把多个 route option 表达为“效率优先 / 体验优先 / 少排队优先”等多方案
4. 如存在 warnings 或 constraints 未满足，必须在 notes 中解释冲突原因和可放宽建议
